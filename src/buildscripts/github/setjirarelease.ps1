<#
    ...
    .EXAMPLE
        .\SetJiraRelease.ps1 -Username "myusername" -Password "mypassword" -Version "1.0.0" -PreVersion "1234"
        Create release with PreVersion

    .EXAMPLE
        .\SetJiraRelease.ps1 -Username "myusername" -Password "mypassword" -Version "1.0.0" -Released
...
param(
    ...
    [Parameter(Mandatory = $false)]
    [string] $JiraProject = "MT",

    [Parameter(Mandatory = $false)]
    [string[]] $StoryWithSubtaskIssue = "MT-1032"
)

# Version should be without v-prefix, remove in case it's passed incorrectly
if ($Version -like "v*") {
...
$releaseName = $packageName = "$DefaultPackage $Version"
if ($PreVersion) {
    $packageName += "-pre-$PreVersion"
}
...
$gitPreviousVersionListCommand = "git for-each-ref refs/tags/ --count=5 --sort=-version:refname --format=""%(refname:short)"""
...
$releaseTaskIssueTypes = @(1, 3)
$releaseTaskIssueStatuses = @("Resolved", "Testing", "Closed")

$releaseStoryIssueTypes = @(10001, 10003)
$releaseStoryIssueStatuses = @("Closed")

$issuesWithNugetPackageVersion = @(1, 3, 10001, 10003, 10101)
